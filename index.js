...
bot.onText(/\/deadline (\d+) (\d{2}\.\d{2}) (\d{2}:\d{2})/, (msg, match) => {
  ...
  if (msg.from.id !== task.authorId) {
    return bot.sendMessage(chatId, "⛔ Лише постановник може змінювати дедлайн!");
  }
  ...
  const deadlineStr = moment(newDeadline).tz(TIMEZONE).format("DD.MM, HH:mm");
  bot.sendMessage(chatId, `✅ Дедлайн задачі #${task.id} змінено на ${deadlineStr}`);

  // Повідомлення виконавцю, якщо він є
  if (task.takenById && task.takenById !== msg.from.id) {
    safeSendMessage(
      task.takenById,
      `⚡ Дедлайн задачі #${task.id} "${task.title}" змінено на ${deadlineStr}`
    ).catch(() => console.log("Не вдалось надіслати виконавцю"));
  }
});
...
/deadline N DD.MM HH:mm - змінити дедлайн задачі
...
⚠️ Увага:
- Якщо задача відправлена у групу і користувач не зареєстрований у боті, кнопки "🏃, ✅, 🗑️, ✏️" можуть пропадати після натискання.  
- У таких випадках рекомендується користуватися командами (/take, /done, /delete, /deadline) замість кнопок.  
- Також можна використати команду /tasks, щоб подивитися всі задачі та натиснути кнопку "✅ Виконано" для потрібної задачі, якщо не зручно писати командами.
- У приватних повідомленнях кнопки працюють стабільно, і там можна налаштувати своє нагадування.
